...
export default function Contact() {
  return (
    <section id="contact" aria-label="Contact Section" className="min-h-screen pt-24 pb-24 bg-neutral-950 text-white relative">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ hidden: {}, show: { transition: { staggerChildren: 0.15 }}}} className="space-y-6">
            {[
              ...
              <motion.div key={i} variants={{ hidden: { opacity: 0, y: 30 }, show: { opacity: 1, y: 0 }}} whileHover={{ y: -6, scale: 1.01 }} className="p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl transition hover:shadow-lg hover:shadow-yellow-500/20">
          ...
          {/* Contact Form */}
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} className="p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl transition hover:shadow-lg hover:shadow-yellow-500/20">
            <form className="space-y-4">
              ...
              <button className="w-full py-3 rounded-lg bg-yellow-500 text-black font-semibold hover:bg-yellow-400 transition shadow-lg shadow-yellow-500/20">Send Message</button>
            </form>
          </motion.div>
